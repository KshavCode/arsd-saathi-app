...
export default function HomeTab({ route, navigation }) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <ScrollView  showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent} refreshControl={
          <RefreshControl
      ...
      >
        ...
        <View style={[styles.heroContainer, { backgroundColor: theme.primary, borderColor: theme.primary+'50' }]}>
          <TouchableOpacity style={styles.heroMainRow} onPress={()=>navigation.navigate("Details")} activeOpacity={.5}>
            <View style={styles.heroTextContent}>
                <Text style={[styles.heroValue, { color: theme.card }]}>{userData.name} - {userData.rollNo}</Text>
                <Text style={[styles.heroLabel, { color: theme.card + 'BB' }]}>{userData.enrollmentNumber}</Text>
            </View>
            <Ionicons name="chevron-forward" size={30} color={theme.card} />
          </TouchableOpacity>        
                  	...
                  	>
                  		<View style={styles.cardHeaderRow}>
                  		  <View style={[styles.timetablePill, { backgroundColor: theme.background }]}>
                  		    ...
                  		    <Text style={[styles.timetablePillText, { color: theme.text }]}>
                  		      {clsItem.slot}
                  		    </Text>
                  		  </View>
												{clsItem.label && (
                    	...
                    	<Text style={[styles.mainCardSubject, { color: theme.text }]} numberOfLines={2}>{clsItem.subject}</Text>
                ...
                <View style={styles.paginationIndicatorRow}>
                  {todaysRemainingClasses.map((_, dotIdx) => {
                    ...
                    return (
                      <View 
                        key={'dot-' + dotIdx} 
                        style={[ styles.pagerDot, { backgroundColor: isDotActive ? theme.primary : theme.separator, width: isDotActive ? 20 : 6} ]} 
                      />
                    );
                  })}
                </View>
