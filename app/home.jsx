...
export default function HomeTab({ route, navigation }) {
  ...
  const actions = [
    ...
    { text: 'HelpDesk', position: 8, icon: 'call', navigate: 'Helpdesk' }
  ];
      ...
      <>
        {isOpen && (
          <View style={styles.backdrop}>
            <TouchableWithoutFeedback onPress={() => setIsOpen(false)} accessibilityRole="button" accessibilityLabel="Close menu">
              <View style={{ flex: 1 }} />
        ...
        <View style={[styles.fabContainer, { bottom: bottomPadding }]}>
            ...
            <Animatable.View animation="fadeInUp" duration={200} style={styles.fabActionsContainer}>
              {actions.map((action) => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                  onPress={() => { setIsOpen(false); navigation.navigate(action.navigate); }}
                  accessibilityRole="button"
                  accessibilityLabel={`Navigate to ${action.text}`}
                >
                  <View style={[styles.fabLabel, { backgroundColor: theme.card }]} importantForAccessibility="no-hide-descendants">
                    <Text style={[styles.fabLabelText, { color: theme.text }]}>{action.text}</Text>
                  </View>
                  <View style={[styles.fabMiniIcon, { backgroundColor: theme.card }]} importantForAccessibility="no-hide-descendants">
          ...
          >
            <Animatable.View transition="rotate" duration={500} style={{ transform: [{ rotate: isOpen ? '90deg' : '0deg' }] }} importantForAccessibility="no-hide-descendants">
  ...
  const getTodaysRemainingClasses = (timetableData) => {
    ...
    return sorted.filter(cls => parseTimeToMinutes(cls.slot) > (currentMinutes - 10));
  };
  ...
  const loadData = useCallback(async () => {
    try {
      const [basicRaw, credsRaw, attRaw, mentorRaw, timestampRaw, timetableRaw] = await Promise.all([
        AsyncStorage.getItem('BASIC_DETAILS'), AsyncStorage.getItem('USER_CREDENTIALS'), AsyncStorage.getItem('ATTENDANCE_DATA'),
        AsyncStorage.getItem('MENTOR_DATA'), AsyncStorage.getItem('DATA_TIMESTAMP'), AsyncStorage.getItem('TIMETABLE_DATA')
      ]);
      ...
      if (timetableRaw) setTodaysRemainingClasses(getTodaysRemainingClasses(JSON.parse(timetableRaw)));
    ...
    try {
      ...
      if (attRaw) {
        ...
        if (data && !data.theory) {
          ...
          Alert.alert("App Updated 🚀", "We've upgraded how attendance is tracked. Please log in again to sync your new data.", [{ text: "OK", onPress: () => navigation.reset({ index: 0, routes: [{ name: 'Login' }] }) }]);
        }
      }
  ...
  const handleSyncError = (errorMsg) => { setIsSyncing(false); console.warn("Background sync failed:", errorMsg); };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      {/* THEME MODAL */}
      <Modal visible={showThemeModal} transparent animationType="fade" onRequestClose={() => setShowThemeModal(false)} accessibilityViewIsModal={true}>
        <TouchableOpacity style={styles.modalBackdrop} activeOpacity={1} onPressOut={() => setShowThemeModal(false)} accessibilityRole="button" accessibilityLabel="Close theme selection">
          <View style={[styles.modalListContainer, { backgroundColor: theme.background, width:'85%' }]} onStartShouldSetResponder={() => true}>
            <Text style={[styles.modalListHeader, { color: theme.text, backgroundColor: theme.card }]} accessibilityRole="header">Select Theme:</Text>
            <ScrollView style={{ maxHeight: 350 }}>
              {Object.keys(Colors).map((name, index) => (
                <TouchableOpacity key={name} style={[styles.dropdownItem, {backgroundColor: themeName === name ? theme.card+'70' : theme.background}]} onPress={() => setThemeName(name)} accessibilityRole="button" accessibilityState={{ selected: themeName === name }} accessibilityLabel={`${name} theme`}>
                  <Animatable.Text style={{ fontSize:18, color: themeName === name ? theme.primary : theme.text, fontWeight: themeName === name ? '800' : '500'}} animation='fadeInUp' duration={300} delay={index*50} useNativeDriver importantForAccessibility="no">{name}</Animatable.Text>
                  {themeName === name && <Ionicons name="checkmark" size={16} color={theme.primary} importantForAccessibility="no" />}
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>
      ...
      <Modal animationType="fade" transparent={true} visible={showLogoutModal} onRequestClose={() => setShowLogoutModal(false)} statusBarTranslucent={true} accessibilityViewIsModal={true}>
        <TouchableOpacity style={styles.modalBackdropCenter} activeOpacity={1} onPressOut={() => setShowLogoutModal(false)}>
          <View style={[styles.modalOverlay, { backgroundColor: theme.modalOverlay }]} onStartShouldSetResponder={() => true}>
            <View style={[styles.modalContent, { backgroundColor: theme.card }]}>
              <View style={[styles.modalIconContainer, { backgroundColor: theme.error + '20' }]} importantForAccessibility="no-hide-descendants"><Ionicons name="alert" size={36} color={theme.error} /></View>
              <Text style={[styles.modalTitle, { color: theme.text }]} accessibilityRole="header">Are you sure?</Text>
              <View style={styles.modalActions}>
                <TouchableOpacity style={{flexDirection:'row', gap:7, alignItems: 'center'}} onPress={() => setDeleteTimetable(!deleteTimetable)} activeOpacity={1} accessibilityRole="checkbox" accessibilityState={{ checked: deleteTimetable }}>
                  <CheckBox value={deleteTimetable} onValueChange={setDeleteTimetable} color={deleteTimetable ? theme.primary : undefined} importantForAccessibility="no" />
                ...
                <View style={[styles.modalActions, {marginTop: 25}]}>
                  <TouchableOpacity style={[styles.modalButtonPrimary, { backgroundColor: theme.error }]} onPress={executeLogout} accessibilityRole="button">
                  ...
                  <TouchableOpacity style={[styles.modalButtonSecondary, { borderColor: theme.primary }]} onPress={() => setShowLogoutModal(false)} accessibilityRole="button">
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[theme.primary]} tintColor={theme.primary} />}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.topIconContainer}>
            <TouchableOpacity onPress={()=>setShowThemeModal(true)} hitSlop={{top: 10, bottom: 10, left: 10, right: 10}} accessibilityRole="button" accessibilityLabel="Change App Theme">
              <Ionicons name='color-palette' size={25} color={theme.primary} importantForAccessibility="no" />
            </TouchableOpacity>
            <TouchableOpacity onPress={handleLogout} hitSlop={{top: 10, bottom: 10, left: 10, right: 10}} accessibilityRole="button" accessibilityLabel="Logout of application">
              <Ionicons name='log-out-outline' size={25} color={theme.error} importantForAccessibility="no" />
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={[styles.heroContainer, { backgroundColor: theme.primary, borderColor: theme.primary+'50' }]}>
          <TouchableOpacity style={styles.heroMainRow} onPress={()=>navigation.navigate("Details")} activeOpacity={.5} accessible={true} accessibilityRole="button" accessibilityLabel={`Student Profile. ${userData.name}, Roll Number ${userData.rollNo}, Enrollment Number ${userData.enrollmentNumber}. Double tap to view full profile.`}>
            <View style={styles.heroTextContent} importantForAccessibility="no-hide-descendants">
                <Text style={[styles.heroValue, { color: theme.card }]}>{userData.name ? titleCase(userData.name.toLowerCase()) : userData.name} - {userData.rollNo}</Text>
                <Text style={[styles.heroLabel, { color: theme.card + 'BB' }]}>{userData.enrollmentNumber}</Text>
            </View>
            <Ionicons name="chevron-forward" size={30} color={theme.card} importantForAccessibility="no" />
          </TouchableOpacity>        
          <View style={[styles.heroDivider, { backgroundColor: theme.separator }]} />
          <View style={styles.extraColumn} accessible={true} accessibilityLabel={`Assigned Mentor: ${userData.mentor_name}`}>
            <View style={styles.mentorRow} importantForAccessibility="no-hide-descendants">
              ...
              <View style={{marginLeft: 12, flex: 1}}>
                ...
                <Text style={[styles.mentorName, { color: theme.card }]}>{userData.mentor_name ? titleCase(userData.mentor_name.toLowerCase()) : userData.mentor_name}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        {/* ATTENDANCE */}
        <TouchableOpacity style={[styles.heroContainer, { backgroundColor: isAttendanceLow ? theme.error+'1A' : theme.success+'20', borderColor: isAttendanceLow ? theme.error : theme.success }]} activeOpacity={.5} onPress={()=>navigation.navigate('Attendance')} accessible={true} accessibilityRole="button" accessibilityLabel={`Overall Theory Attendance is ${userData.percent_attendance} percent. ${isAttendanceLow ? 'Warning: Attendance is below 67 percent.' : ''} Double tap for details.`}>
          <View style={styles.heroMainRow} importantForAccessibility="no-hide-descendants">
            <View style={[styles.heroIconBox, { backgroundColor: isAttendanceLow ? theme.error : theme.success }]}><Ionicons name="pie-chart" size={25} color='#FFF' /></View>
          ...
          {isAttendanceLow && <Text style={[styles.warningText, { color: theme.error }]} importantForAccessibility="no">* Below 67%.</Text>}
        </TouchableOpacity>
                ...
                const isCurrentActive = (idx === activeCardIndex);
                  return (
                    <TouchableOpacity 
                      ...
                      style={[ styles.heroContainer, styles.carouselCardInstance, { width: CARD_WIDTH, marginRight: idx === todaysRemainingClasses.length - 1 ? 0 : CARD_GAP, backgroundColor: theme.card, borderColor: isCurrentActive ? theme.primary : theme.separator } ]} 
                      activeOpacity={0.8}
                      accessible={true}
                      accessibilityRole="button"
              ...
              {todaysRemainingClasses.length > 1 && (
                <View style={styles.paginationIndicatorRow} accessible={true} accessibilityLabel={`Showing class ${activeCardIndex + 1} of ${todaysRemainingClasses.length}`}>
                  {todaysRemainingClasses.map((_, dotIdx) => (
                    <View key={'dot-' + dotIdx} style={[ styles.pagerDot, { backgroundColor: (dotIdx === activeCardIndex) ? theme.primary : theme.separator, width: (dotIdx === activeCardIndex) ? 20 : 6} ]} importantForAccessibility="no" />
                  ))}
                </View>
              )}
          </View>
...
const styles = StyleSheet.create({
  ...
  carouselSectionWrapper: { marginBottom: 20 },
  carouselCardInstance: { borderRadius: 10, padding: 15, borderWidth: 1.5, marginBottom: 5},
  cardHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  timetablePill: { flexDirection: 'row', alignItems: 'center', paddingVertical: 6, paddingHorizontal: 5, borderRadius: 10},
  timetablePillText: { fontSize: 12, fontWeight: '800', letterSpacing: 0.3 },
  mainCardSubject: { fontSize: 18, fontWeight: '900', letterSpacing: -0.5, marginBottom: 5 },
