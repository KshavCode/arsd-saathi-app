...
// Helper
const titleCase = (str) => str ? str.toLowerCase().replace(/\b\w/g, s => s.toUpperCase()) : '';
    ...
    >
      <View style={styles.cardHeader} importantForAccessibility="no-hide-descendants">
        <View style={styles.headerTextCtx}>
          <Text style={[styles.teacherName, { color: theme.text }]}>{titleCase(facultyName)}</Text>
          <Text style={[styles.teacherCode, { color: theme.secondary }]}>{data.FAC_CODE ? `Faculty Code: ${data.FAC_CODE}` : "No Code"}</Text>
        </View>
      </View>
      ...
      <View style={styles.cardBody} importantForAccessibility="no-hide-descendants">
        ...
        <Text style={[styles.paperName, { color: theme.text }]}>{paperName}</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: theme.background+'70' }]}>
            ...
            <Text style={[styles.badgeText, { color: theme.primary }]}>{data["Paper Code"] || data.PAPER_ID || "No ID"}</Text>
          </View>
          {section && (
            <View style={[styles.badge, { backgroundColor: theme.background+'70' }]}>
              <Ionicons name="people-outline" size={12} color={theme.primary} style={{ marginRight: 4 }} />
              <Text style={[styles.badgeText, { color: theme.primary }]}>Sec: {section}</Text>
            </View>
          )}
        </View>
      </View>
...
export default function AttendanceScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.tabContainer, { backgroundColor: theme.card, borderColor: theme.secondary }]} accessibilityRole="tablist">
        {['basic', 'faculty'].map((tab) => (
          <TouchableOpacity
            ...
            accessibilityLabel={`${tab} details tab`}
          >
      ...
      {activeTab === 'basic' ? <BasicView /> : <FacultyView />}
      <OfflineBanner />
    </SafeAreaView>
  );
}
...
function BasicView() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        let combinedData = basicRaw ? JSON.parse(basicRaw) : {};
        if (credsRaw) {
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1, paddingBottom: 30 }} showsVerticalScrollIndicator={false}>
      {loading ? (
        <ActivityIndicator size="large" color={theme.primary} style={{ marginTop: 50 }} accessibilityRole="progressbar" accessibilityLabel="Loading personal details" />
      ) : (
        <View style={{ backgroundColor: theme.card, borderColor: theme.background }}>
          {profileData ? displayFields.map((field, index) => (
            ...
            <View style={{ padding: 40, alignItems: 'center' }} accessible={true} accessibilityRole="alert" accessibilityLabel="Warning: No details found.">
...
function FacultyView() {
  ...
  return (
    <>
      {loading ? (
        <ActivityIndicator size="large" color={theme.primary} style={{ marginTop: 50 }} accessibilityRole="progressbar" accessibilityLabel="Loading faculty data" />
      ) : facultyList.length === 0 ? (
        <View style={styles.centerContainer} accessible={true} accessibilityRole="alert" accessibilityLabel="No faculty details found.">
          ...
          <Text style={{ color: theme.secondary }} importantForAccessibility="no">No faculty details found.</Text>
        </View>
      ) : (
        <FlatList 
          data={facultyList}
