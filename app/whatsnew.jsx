...
export default function WhatsNewTab({ navigation }) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, paddingBottom: 40, paddingTop: 10 }} showsVerticalScrollIndicator={false}>
        <View 
        ...
        >
          ...
          <Text style={[styles.sectionTitle, { color: theme.text }]} importantForAccessibility="no">New Features</Text>
        </Animatable.View>
        
        <View style={styles.listContainer}>
          {UPDATES_DATA.updates.map((update, index) => (
            <UpdateItem key={`update-${index}`} item={update} theme={theme} delay={300 + (index * 100)} />
          ))}
        </View>
        ...
        <Animatable.View 
          ...
          accessible={true}
          accessibilityRole="header"
          accessibilityLabel="Bug Fixes"
        >
          <Ionicons name="bug" size={20} color={theme.error} style={{ marginRight: 8 }} importantForAccessibility="no" />
          <Text style={[styles.sectionTitle, { color: theme.text }]} importantForAccessibility="no">Fixes</Text>
        </Animatable.View>
        
        <View style={styles.listContainer}>
          {UPDATES_DATA.fixes.map((fix, index) => (
            <UpdateItem key={`fix-${index}`} item={fix} theme={theme} isFix={true} delay={900 + (index * 100)} />
          ))}
        </View>
      </ScrollView>
      <OfflineBanner />
